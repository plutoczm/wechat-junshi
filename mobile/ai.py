...
friend in CURRENT_MESSAGE. The friend is NOT an advice customer and a third person
mentioned by the friend is NOT the recipient. Keep the owner's factual knowledge
separate from assumptions. Never invent the owner's whereabouts, availability,
feelings, relationship status, consent or promises. Ask instead when necessary.
Use warm, natural, brief Chinese. No manipulation, pressure, harassment or deception
about material facts. Do not insist on replying to every emoji. Unknown stickers
are unknown, not a guessed emotion. Sensitive commitments require the owner's review.
...
not independently verified facts. Timestamps marked null are unknown. Do not mistake
an unsent draft for a sent message. Output strict JSON only:
...
def prompt(snapshot):
    ...
    if len(current) > 4000:
        raise Problem("Select a message of at most 4000 characters for generation")
    budget, records = 12000, []
    candidates = sorted(snapshot["records"], key=lambda r: (r.get("score", 0), r["occurred_at"] or r["observed_at"]), reverse=True)
    for row in candidates:
        if row["id"] == snapshot["current"]["id"]:
            continue
        excerpt = row["content"][:800]
        if len(excerpt) > budget:
            break
        budget -= len(excerpt)
        records.append({"role": row["role"], "text": excerpt, "truncated": len(row["content"]) > 800,
                        "account_ref": row["account_id"], "occurred_at": row["occurred_at"], "origin": row["origin"]})
    notes, budget = [], 4000
    for n in snapshot["notes"]:
        if len(n["content"]) > budget:
            break
        budget -= len(n["content"])
        notes.append({"text": n["content"], "account_ref": n["account_id"], "owner_supplied": True})
    return [{"role": "system", "content": SYSTEM}, {"role": "user", "content": json.dumps({
        "CURRENT_MESSAGE": current, "current_account_ref": snapshot["account_id"],
        "same_person_history": records, "owner_notes": notes,
        "optional_relationship_reference": knowledge(current),
    }, ensure_ascii=False)}]
